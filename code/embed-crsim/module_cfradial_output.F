module module_cfradial_output
  ...
  type(config_output_type) :: config_zhh_hail ! Added by BWK, 3/9/2022
  ...
  !type(config_output_type) :: config_WRF_QHAIL   ! Added by BWK, 3/9/2022
  !type(config_output_type) :: config_WRF_QNCLOUD   ! Added by BWK, 3/9/2022
  ...
  !type(config_output_type) :: config_WRF_QNHAIL  ! Added by BWK, 3/9/2022
...
!         &                  config_zhh_hail,       &    ! Added by BWK, 3/9/2022
         ...
         &                  config_WRF_QSNOW,      &
         &                  config_WRF_QGRAUP,     &
         !&                  config_WRF_QHAIL,      &   ! Added by BWK, 3/9/2022
         !&                  config_WRF_QNCLOUD,    &   ! Added by BWK, 3/9/2022
         &                  config_WRF_QNRAIN,     &
         &                  config_WRF_QNICE,      &
         &                  config_WRF_QNSNOW,     &
         &                  config_WRF_QNGRAUPEL,  &
         !&                  config_WRF_QNHAIL,     &   ! Added by BWK, 3/9/2022
         &                  config_WRF_U,          &
         &                  config_WRF_V,          &
         &                  config_WRF_W
...
!!!! End addition by B. Klotz
 ...
 !   endif
    ...
    ! Added by BWK, 3/9/2022
    !if (config_zhh_hail%output) then
    !    ierr = nf90_def_var ( self%ncid, "Zhh_hail", NF90_FLOAT, dimids=(/range_dimid,time_dimid/), varid=config_zhh_hail%varid, &
    !     &                deflate_level=2, shuffle=.true. )
    !    ierr = nf90_put_att ( self%ncid, config_zhh_hail%varid, "standard_name", "Zhh hail")
    !    ierr = nf90_put_att ( self%ncid, config_zhh_hail%varid, "long_name", "Zhh contribution from hail")
    !    ierr = nf90_put_att ( self%ncid, config_zhh_hail%varid, "units", "dBZ")
    !    ierr = nf90_put_att ( self%ncid, config_zhh_hail%varid, "_FillValue", -9999.0_4 )
    !    ierr = nf90_put_att ( self%ncid, config_zhh_hail%varid, "coordinates", "elevation azimuth range heading roll pitch rotation tilt" )
    !endif
...
!---------------------------------------------------------------------------------------------
  ...
  subroutine cfradial_prepare_metadata(self, volume)
    implicit none
    class(cfradial_type), intent(inout) :: self
...
! #define _KWM_NOPE1_ 1
    ...
    do while ( associated ( ptr%next ) )
        ...
        elseif ( ptr%source == "WRF" ) then
            ...
            if ( ptr%field_name == "RHO_DS"      .and. .not. config_WRF_RHO_DS%output ) cycle   ! Added by BWK, 3/14/2022
            ...
            !if ( ptr%field_name == "QNCLOUD"     .and. .not. config_WRF_QNCLOUD%output ) cycle   ! Added by BWK, 3/9/2022
...
#ifdef _KWM_NOPE1_
            !if ( ptr%field_name == "QHAIL"       .and. .not. config_WRF_QHAIL%output ) cycle   ! Added by BWK, 3/9/2022
            !if ( ptr%field_name == "QNHAIL"      .and. .not. config_WRF_QNHAIL%output ) cycle  ! Added by BWK, 3/9/2022
#endif
...
!---------------------------------------------------------------------------------------------
  ...
  subroutine write_volume ( self , volume , primary_axis )
    ...
    write(*,'("time_coverage_end = ", A, " + ", I8, " = ", A)') volume%reference_time,  int(volume%time(volume%nrays)), volume%time_coverage_end
...
!!!! End addition by B. Klotz
    ...
    !if (config_zhh_hail%output) then
    !    arr => volume%point_to_data("Zhh_hail")
    !    ierr = nf90_put_var(self%ncid, config_zhh_hail%varid, arr(1:volume%ngates,1:volume%nrays))
    !    call error_handler(ierr, "Problem put Zhh_hail")
    !    nullify(arr)
    !endif
...
!---------------------------------------------------------------------------------------------
  ...
  function volume_initialize ( number, reference_time, current_time_seconds, &
    ...
    use kwm_date_utilities, only : geth_newdate
    implicit none
    type(volume_type), target :: volume_initialize
    ...
    write(*,'("volume_initialize:  time_coverage_start = ", A, " + ", I8, " = ", A)') volume_initialize%reference_time, current_time_seconds, volume_initialize%time_coverage_start
    ...
    ptr => volume_initialize%root
    do while ( associated ( ptr%next ) ) 
        ptr => ptr%next
        deallocate(ptr%data)
        volume_initialize%root%next => ptr%next
        nullify(ptr)
        ptr => volume_initialize%root
    enddo
    ...
    call volume_initialize%initialize_new_field ( -9999  , "RHO_DS", source="WRF", description = "rho_ds",         units = "kg/m^3" )
