...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  INTEGER, PARAMETER :: HH=4, VV=5
   ...
   REAL(KIND=RKIND), PARAMETER :: OMEGA_E = 7.292e-5
...
!      lon = deltalon + proj%stdlon
   ...
   SUBROUTINE llij_rotlatlon(lat, lon, proj, i_real, j_real)
      ...
      dphd = proj%phi/REAL((proj%jydim-1)/2,KIND=RKIND)
      dlmd = proj%lambda/REAL(proj%ixdim-1,KIND=RKIND)
...
!!! Added next line as a Kludge - not yet understood why needed
      ...
      i = REAL(ii,KIND=RKIND)
      j = REAL(jj,KIND=RKIND)
   ...
   SUBROUTINE lggaus( nlat, cosc, gwt, sinc, colat, wos2 )
      ...
      REAL(KIND=RKIND) (KIND=HIGH) , DIMENSION(nlat) :: cosc , gwt , sinc , colat  , wos2 
      REAL(KIND=RKIND) (KIND=HIGH) , PARAMETER       :: pi = 3.141592653589793
      ...
      INTEGER :: nzero, i, j
      REAL(KIND=RKIND) (KIND=HIGH) :: fi, fi1, a, b, g, gm, gp, gt, delta, c, d
   ...
   SUBROUTINE lgord( f, cosc, n )
      ...
      REAL(KIND=RKIND) (KIND=HIGH) :: s1, c4, a, b, fk, f, cosc, colat, c1, fn, ang
      INTEGER :: n, k
