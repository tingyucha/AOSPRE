...
set(_C_flag --cflags)
set(_Fortran_flag --fflags)
set(_CXX_flag --cflags)
...
foreach( _comp IN LISTS _search_components )
  ...
  #Use nc-config to set per-component compile flags if possible
  netcdf_config( ${NetCDF_${_comp}_CONFIG_EXECUTABLE} ${_${_comp}_flag} _val)
  if(_val)
    string(REPLACE " " ";" _val ${_val})
    set(NetCDF_${_comp}_FLAGS ${_val})
  endif()
  message(STATUS "NetCDF_${_comp}_FLAGS: ${_val}")

  #Use nc-config to set per-component link flags if possible
  netcdf_config( ${NetCDF_${_comp}_CONFIG_EXECUTABLE} ${_${_comp}_libs_flag} _val)
  if(_val)
    string(REPLACE " " ";" _val ${_val})
    set(NetCDF_${_comp}_LDFLAGS ${_val})
  endif()
  message(STATUS "NetCDF_${_comp}_LDFLAGS: ${_val}")
  ...
  if( NetCDF_${_comp}_LIBRARIES AND NetCDF_${_comp}_INCLUDE_DIRS )
    ...
    if (NOT TARGET NetCDF::NetCDF_${_comp})
      ...
      set_target_properties(NetCDF::NetCDF_${_comp} PROPERTIES
        ...
        INTERFACE_COMPILE_OPTIONS "${NetCDF_${_comp}_FLAGS}"
        INTERFACE_LINK_LIBRARIES ${NetCDF_${_comp}_LIBRARIES}
        INTERFACE_LINK_OPTIONS "${NetCDF_${_comp}_LDFLAGS}"
        )
    endif()
  endif()
endforeach()
